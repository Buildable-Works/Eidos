...
- **A collection item** (a spec, most often) — reshape it toward its collection's flavor shape in `.eidos/shapes/` (the collection's default unless the draft's `flavor` says otherwise), with frontmatter from `.eidos/Schema.md`. The most structured case.
- **A canonical top-level doc** (Architecture, Audience, Criteria, Market) — reshape it toward its matching **template** in `.eidos/templates/`. Loose prose; light inline frontmatter.
- **A free-form top-level doc** the registry has added (a Roadmap, a Vision, a Glossary) — there is **no shape and none is expected**. Don't reach for a template or check it against one. Organize the thoughts into the house style — readable headings, tables, lists, links over bare names — and maintain the light top-level-doc frontmatter (`type`, `title`, `tags`, `created`, `modified`). Same spirit as the rules, applied to a doc that is the owner's own structure.
...
- **`.eidos/shapes/`** — collection body shapes, per flavor (`spec.full.md`, `spec.micro.md`).
- **`.eidos/templates/`** — the one-of-each top-level-doc templates (`Architecture.md`, …). A free-form top-level doc has none; that's expected.
- **`.eidos/Schema.md`** — the property contract, for an item's frontmatter.
- **`.eidos/Registry.md`** — the naming convention (so any link you create uses the registry's filename style — `%20` only in a Title Case registry), and the collections with their flavors, so you know which shape an item follows.
...
2. **Get the form from the registry.** Open the matching form (the item's flavor shape in `.eidos/shapes/` — its collection's default unless its `flavor` says otherwise — for a collection item; the top-level-doc template in `.eidos/templates/` for one of the four) and `.eidos/Schema.md` for an item's frontmatter contract. **A free-form top-level doc has no shape** — skip this and organize by the doc's own logic, keeping only the light frontmatter convention (`type`, `title`, `tags`, `created`, `modified`).
3. **Sort the existing content into sections, using their words.** For a collection item or top-level doc, follow its shape — for a spec that's Intent; Open Questions & Assumptions; Behaviors & Acceptance Criteria; Out of Scope; Dependencies; Testing; Constraints & Decisions (another collection's shape will differ). A loose sentence about what it won't do goes to Out of Scope; an uncertainty goes to Open Questions; an observable outcome becomes an `AC{n}:`. For a free-form top-level doc there are no prescribed sections — group the content under the owner's own headings (or ones you draw from the draft and flag), in a sensible order, without forcing it into the spec sections.
...
5. **Fill only derivable frontmatter.** For a collection item, generate the frontmatter keys from `.eidos/Schema.md`'s required properties; fill `title` from the document's heading and `created`/`modified` to today. Leave `id`, `owner`, `domain`, `status`, and any custom-required value for the user where you can't derive them honestly — don't guess a `status`. For a top-level doc (canonical or free-form) there's no Schema to generate from — keep the light top-level-doc frontmatter (`type`, `title`, `tags`, `created`, `modified`), filling `title` and the dates and leaving the rest.
